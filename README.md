...
* Batch Consumer - 큐에 적재된 데이터를 감시하며, 일정 건수(1000건) 단위로 데이터를 묶어 DB에 일괄 저장
...
**✅ 쿼리 분석을 통한 트래픽 자동 라우팅 시연**

**1) 단순 조회 (`SELECT`)** - ProxySQL이 읽기 트래픽으로 판단하여 `db-replica-1`, `db-replica-2`로 분산 라우팅합니다.
<p align="center">
  <img src="./images/select.png" width="80%"/>
</p>

**2) 쓰기 의도 쿼리 (`SELECT ... FOR UPDATE`)** - ProxySQL이 쓰기 트래픽으로 판단하여 Master인 `db-source`로만 라우팅합니다.
<p align="center">
  <img src="./images/for-update.png" width="80%"/>
</p>
...
* MySQL: Master-Slave 복제 구조 (1 Master - 2 Replica 고가용성 구성)
